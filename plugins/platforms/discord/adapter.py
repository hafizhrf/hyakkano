...
class DiscordAdapter(BasePlatformAdapter):
    ...
    async def _handle_clear_chat_slash(
        self, interaction: "discord.Interaction", amount: int,
    ) -> None:
        """Delete recent non-pinned messages in the invoking text channel.

        This is intentionally a native Discord operation rather than an LLM
        command: deleting chat history is destructive, must be authorized
        before acknowledging the interaction, and should never depend on a
        model deciding what ``clear`` means. Discord bulk deletion only works
        for recent messages; ``TextChannel.purge`` uses the platform's safe
        fallback for older messages where possible.
        """
        command_text = f"/clear-chat {amount}"
        if not await self._check_slash_authorization(interaction, command_text):
            return

        if amount < 1 or amount > 1000:
            await interaction.response.send_message(
                "Amount must be between 1 and 1000.", ephemeral=True,
            )
            return
        ...
        if channel is None or not hasattr(channel, "purge"):
            await interaction.response.send_message(
                "`/clear-chat` can only be used in a server text channel.",
                ephemeral=True,
            )
            return
        ...
        try:
            ...
            deleted = await channel.purge(
                ...
                bulk=True,
                reason=f"/clear-chat requested by {getattr(interaction.user, 'id', 'unknown')}",
            )
            deleted_count = len(deleted)
            await interaction.edit_original_response(
                content=f"Deleted {deleted_count} recent message(s). Pinned messages were kept.",
            )
            logger.info(
                "[Discord] /clear-chat deleted=%d requested=%d channel=%s user=%s",
                deleted_count,
                amount,
                getattr(channel, "id", "?"),
            ...
            logger.warning("[Discord] /clear-chat denied: missing Manage Messages permission")
            try:
                await interaction.edit_original_response(
            ...
            logger.warning("[Discord] /clear-chat failed: %s", e, exc_info=True)
            try:
                await interaction.edit_original_response(
                    content="I couldn't clear the channel. Check my permissions and try again.",
                )
            except Exception:
                pass
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="clear-chat", description="Delete recent non-pinned messages in this channel")
        @discord.app_commands.describe(amount="Number of recent messages to delete (1-1000)")
        async def slash_clear_chat(interaction: discord.Interaction, amount: int):
            await self._handle_clear_chat_slash(interaction, amount)
    ...
    async def _handle_message(self, message: DiscordMessage, role_authorized: bool = False) -> None:
        ...
        if is_thread:
            ...
            # When DISCORD_IGNORE_THREADS is set, this bot never responds inside ANY
            # thread (used when a sibling bot — e.g. Shaula — owns thread execution and
            # this bot should stay out, even with require_mention=false in the parent).
            if os.getenv("DISCORD_IGNORE_THREADS", "false").lower() in {"true", "1", "yes", "on"}:
                logger.debug("[%s] Ignoring thread message (DISCORD_IGNORE_THREADS): %s", self.name, thread_id)
                return
        ...
        # invisible to the session transcript.  The same gap exists in a
        # shared free-response channel when a reply gate elects one bot to
        # stay silent: the other bot's answer is heard by the gateway but is
        # deliberately not added to this bot's agent session.  To recover
        # either kind of context, fetch recent channel history and prepend it
        # to the user message.
        #
        ...
        if not _is_dm and self._discord_history_backfill():
            ...
            #   - shared free-response channels that accept other bots.  A
            #     reply-selection plugin may skip those inbound turns, so the
            #     transcript is not actually complete even though the channel
            #     is free-response.
            ...
            _shared_free_response = (
                is_free_channel
                and os.getenv("DISCORD_ALLOW_BOTS", "none").lower().strip() != "none"
            )
            if (_has_mention_gap or _shared_free_response or is_thread) and auto_threaded_channel is None:
