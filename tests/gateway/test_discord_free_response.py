...
@pytest.mark.asyncio
async def test_discord_shared_free_response_backfills_partner_turns(adapter, monkeypatch):
    """A reply gate can skip partner turns, so free-response is not full context."""
    monkeypatch.setenv("DISCORD_REQUIRE_MENTION", "false")
    ...
    adapter._fetch_channel_context = AsyncMock(
        return_value="[Recent channel messages]\n[Hakari [bot]] AFA ID is Anime Festival Asia Indonesia"
    )

    message = make_message(
        channel=FakeTextChannel(channel_id=321),
        content="does Hololive ID attend this event, Karane?",
    )
    ...
    adapter._fetch_channel_context.assert_awaited_once()
    event = adapter.handle_message.await_args.args[0]
    assert event.text == "does Hololive ID attend this event, Karane?"
...
@pytest.mark.asyncio
async def test_clear_chat_deletes_requested_recent_non_pinned_messages(adapter):
    channel = SimpleNamespace(id=321, purge=AsyncMock(return_value=[object(), object()]) )
    ...
    await adapter._handle_clear_chat_slash(interaction, 2)

    channel.purge.assert_awaited_once()
    assert channel.purge.await_args.kwargs["limit"] == 2
    assert channel.purge.await_args.kwargs["bulk"] is True
    check = channel.purge.await_args.kwargs["check"]
    assert check(SimpleNamespace(pinned=False)) is True
    assert check(SimpleNamespace(pinned=True)) is False
    interaction.edit_original_response.assert_awaited_once_with(
        content="Deleted 2 recent message(s). Pinned messages were kept.",
    )
